...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "MJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
try {
const listaDirectorios = readdirSync('./MJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./MJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./MJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./MJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./GataBotSession/', './MJadiBot/']
